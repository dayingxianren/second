...
def calculate_total_time(total_car_distance, drone_distances):
    ...
    def plot_path(points, path, color, label, path_index):
    ...
    for idx, (x, y) in enumerate(path_points):
        plt.text(x, y, str(path_index[idx + 1]), fontsize=9, ha='center', va='center', color='white',
    ...
    plt.subplot(1, 2, 1)
    plt.title('UAV+UGV')
    plot_path(relay_points_drone, car_path, 'blue', 'Car Path', [0] + list(range(1, len(relay_points_drone) + 1)))

    path_index = len(relay_points_drone) + 1  # 从最后一个中继点的索引开始标注无人机配送的序号
    for i, drone_path_group in enumerate(drone_paths):
        for j, path_group in enumerate(drone_path_group):
            for path in path_group:
                ...
                plot_path(packages, path[1:-1], 'cyan', '', list(range(path_index, path_index + len(path) - 2)))
                path_index += len(path) - 2  # 更新下一个无人机配送的起始序号

    # 显示包裹位置和无法配送的包裹
    special_indices = [point['original_index'] for point in special_packages]  # 获取特殊点的索引列表
    ...
    # 绘制非特殊点的包裹
    non_special_indices = [i for i in range(len(packages)) if i not in special_indices]
    ...
    for k in range(min_K, max_K, step):  # 可以根据需要调整K值的范围和步长
        # 聚类以确定无人车+无人机方案的中继点
        kmeans_drone = KMeans(n_clusters=k)
        ...
        # 计算无人车路径（TSP）
        relay_points_drone = np.vstack([car_start, relay_points_drone])
